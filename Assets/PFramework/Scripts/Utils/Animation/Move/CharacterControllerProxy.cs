...
public class CharacterControllerProxy : MoveProxy<CharacterController>
{
    ...
    public override void MoveByTarget(Vector3 worldPosition)
    {
        ...
        if (movementVelocity != Vector3.zero)
        {
            ...
            playerControl.transform.position = Vector3.MoveTowards(playerControl.transform.position, worldPosition, speed * Time.deltaTime);
        }
    }
    ...
    public override void MoveByInput(float inputX, float inputY)
    {
        if (!Mathf.Approximately(inputX, 0f) || !Mathf.Approximately(inputY, 0f))
        {
            ...
            if (isJump)
            {
                mv += jumpForce * Vector3.up * Time.deltaTime;
            }
            else
            {
                //判断是否需要重力
                if (playerControl.isGrounded == false)
                {
                    mv += Gravity * Vector3.up * Time.deltaTime;
                }
            }
