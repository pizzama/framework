    ...
    public class WorldTiler : MonoBehaviour
    {
        ...
        public void TileWorld(TurfData[] turfDataArr, Grid grid, WorldGridData worldGridData, TurfData[,] turfGrid)
        {
            ...
            foreach (var texture in tileTextures)
            {
                ...
                IList<Tile> tiles =  AssetsManager.Instance.LoadFromBundleWithSubResources<Tile>($"{texture.name}_tiles");  //Addressables.LoadAssetsAsync<Tile>($"{texture.name}_tiles", null).WaitForCompletion();
