...
Shader "SFrameworkEffect/Urp/RimColor"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_MainBase("MainBase", 2D) = "white" {}
		_ColorIndensity("ColorIndensity", Float) = 1
		[Toggle(_ISGRAY_ON)] _IsGray("IsGray", Float) = 0
		_GrayCoefficient("GrayCoefficient", Color) = (0.2980392,0.5882353,0.1098039,0)
		[ASEEnd]_Alpah("Alpah", Range( 0 , 1)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma shader_feature_local _ISGRAY_ON
			...
			float4 _MainBase_ST;
			float4 _GrayCoefficient;
			float _ColorIndensity;
			float _Alpah;
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv_MainBase = IN.ase_texcoord3.xy * _MainBase_ST.xy + _MainBase_ST.zw;
				float4 tex2DNode1 = tex2D( _MainBase, uv_MainBase );
				float dotResult13 = dot( tex2DNode1 , _GrayCoefficient );
				float4 temp_cast_0 = (dotResult13).xxxx;
				#ifdef _ISGRAY_ON
				float4 staticSwitch16 = temp_cast_0;
				#else
				float4 staticSwitch16 = ( _ColorIndensity * tex2DNode1 );
				#endif
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = staticSwitch16.rgb;
				float Alpha = _Alpah;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN ) : SV_TARGET
			{
				...
				surfaceDescription.Alpha = _Alpah;
...
ASEEND*/
//CHKSM=4BED088725042D202852A593886E20887EAEE294