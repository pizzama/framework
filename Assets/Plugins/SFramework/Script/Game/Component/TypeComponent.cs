...
public abstract class TypeComponent : MonoBehaviour
{
    ...
    public IEnumerator TypeText(string text)
    {
        ...
        for (var i = 0; i < len; i++)
        {
            ...
            if (text[i] == '<' && i + (1 + 13) < len && text.Substring(i, 2 + 6).Equals("<color=#") && text[i + 14] == '>')
            {
...
public static class TypeTextComponentUtility
{
    ...
    public static void TypeText(this TMP_Text label, string text, float speed = 0.05f, TypeComponent.OnComplete onComplete = null)
    {
